...
def main():
    ...
    parser.add_argument("--compute_taylor",
                        action='store_true',
                        help="Whether to accumulate grad to compute taylor.")
    ...
    if (not args.do_eval) and (not args.compute_taylor):
    ...
    if args.do_eval:
        ...
        for epoch_ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                loss.backward()
                if args.compute_taylor or 'depth_or_width' in args:
