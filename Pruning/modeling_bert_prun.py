...
from hg_transformer.src.transformers.activations import gelu, gelu_new, swish
from configuration_bert_prun import BertConfigPrun as BertConfig
from hg_transformer.src.transformers.file_utils import add_start_docstrings, add_start_docstrings_to_callable
from hg_transformer.src.transformers.modeling_utils import PreTrainedModel, prune_linear_layer
    ...
    def __init__(self, config):
        ...
        if config.emb_hidden_dim == -1:
            ...
            self.word_embeddings1 = nn.Embedding(
                config.vocab_size, config.emb_hidden_dim, padding_idx=0)
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.return_grad = config.return_grad
    ...
    def forward(
        ...
        if self.return_grad:
            return outputs, intermediate_output, self_attention_outputs[0]
        else:
...
class BertEncoder(nn.Module):
    ...
    def save_intermedia_gradient(self, grad):
        self.intermedia_gradients.append(grad)
    def save_attn_gradient(self, grad):
        self.attn_gradients.append(grad)
    ...
    def forward(
        ...
        all_hidden_states = ()
        all_intermedia_outputs = ()
        all_attn_outputs = ()
        all_attentions = ()
        self.intermedia_gradients, self.attn_gradients = [], []
        for i, layer_module in enumerate(self.layer):
            if self.output_hidden_states:
                all_hidden_states = all_hidden_states + (hidden_states,)

            layer_outputs, intermediate_output, attn_output = layer_module(
                hidden_states, attention_mask, head_mask[i], encoder_hidden_states, 
                encoder_attention_mask
            )
            hidden_states = layer_outputs[0]
            all_intermedia_outputs = all_intermedia_outputs + (intermediate_output,)
            all_attn_outputs = all_attn_outputs + (attn_output,)
            if intermediate_output is not None:
                intermediate_output.register_hook(self.save_intermedia_gradient)
            if attn_output is not None:
                attn_output.register_hook(self.save_attn_gradient)
        ...
        if self.return_grad:
            outputs = outputs + (all_intermedia_outputs,) + (all_attn_outputs,)
        return outputs  # last-layer hidden state, (all hidden states), (all attentions), 
                        # (all_intermedia_outputs), (all_attn_outputs)
...
class PrunBertModel(BertPreTrainedModel):
    """
    ...
    Examples::
        ...
        return outputs  # sequence_output, pooled_output, (hidden_states), (attentions), (intermedia_outputs)
...
class PrunBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels

        self.bert = PrunBertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    Examples::
        ...
        return outputs  # (loss), logits, (hidden_states), (attentions), (intermedia_outputs)
